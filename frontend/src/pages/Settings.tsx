    ...
    fields: [
      {
        ...
        label: '影象清晰度',
        type: 'select',
        description: '產圖後會依此縮放輸出尺寸。註：影像模型原生約 2K，選 4K 為放大、不會增加真實細節',
        options: [
      ...
      },
      {
        ...
        label: '顯示頁面標題（頁標）',
        type: 'switch',
        description: '關閉後，非封面頁不會渲染頁面標題，直接以內文為主體排版',
      },
      {
        key: 'title_emphasis',
        label: '標題字級 / 強調度',
        type: 'select',
        description: '控制頁面標題相對內文的字級大小（軟提示，影像模型會盡量遵循）',
        options: [
          { value: 'small', label: '小' },
    ...
    try {
      ...
      if (response.data) {
        ...
        setFormData({
          ...
          show_page_title: response.data.show_page_title ?? true,
          title_emphasis: response.data.title_emphasis || 'medium',
