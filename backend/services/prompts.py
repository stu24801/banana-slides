...
def get_clean_background_prompt() -> str:
    """
    生成純背景圖的 prompt（只去除文字，保留圖示/icon/裝飾元素）
    用於從完整的PPT頁面中提取純背景
    """
    prompt = """\
你是一位專業的圖片文字擦除專家。你的任務是從原始投影片圖片中**只移除文字內容**，輸出一張保留所有設計元素但沒有任何文字的底板圖。
<requirements>
- 移除所有文字（標題、內文、bullet points、標籤、數字說明等）。
- **保留** 所有非文字元素：icon、圖示、插圖、圖形、裝飾圖案、色塊、漸變背景、線條、邊框等。
- 保持原背景設計的完整性（包括漸變、紋理、圖案、線條、色塊等）。
- 對於被文字遮擋的背景區域，要智慧填補使背景保持無縫和完整。
- 輸出圖片的尺寸、風格、配色必須和原圖完全一致。
- 請勿新增任何元素，請勿移除任何非文字元素。
</requirements>

注意：**只移除文字**，所有的 icon、插圖、圖形裝飾都必須完整保留在原位。
"""
